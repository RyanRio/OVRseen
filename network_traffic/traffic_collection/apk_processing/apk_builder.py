...
class ApkBuilder:
	def __init__(self, apk_path: str, keystore_pw: str, inject_frida = False, use_aapt2 = False, downgrade_api=False, inject_internet_perm=False) -> None:
		"""
		apk_path: (absolute) path to APK
		keystore_pw: keystore password for APK signing

		"""
		self.apk_path = apk_path
		self.keystore_pw = keystore_pw
		self.inject_frida = inject_frida
		self.use_aapt2 = use_aapt2
		self.downgrade_api = downgrade_api
	...
	def run(self):
		...
		try:
			if os.path.isdir(WORK_DIR):
				globals.redirect_print_func("[I] Preparing work directory...")
			...
			if self.inject_internet_perm:
				if not "uses-permission: name='android.permission.INTERNET'" in apk_permissions:
					globals.redirect_print_func("[I] APK needs INTERNET permission")
					...
					for line_num in range(0, len(manifest_file_contents)):
						globals.redirect_print_func(manifest_file_contents[line_num])
						if "android.permission.INTERNET" in manifest_file_contents[line_num]:
							...
							with codecs.open(manifest_file_path, 'w', 'utf-8') as f:
								...
								globals.redirect_print_func("\tSuccess: Added Internet Permission into APK")
							break
				else:
					globals.redirect_print_func("\tAlready have internet permission")
			
			if self.downgrade_api:
				...
				globals.redirect_print_func("[I] Change AndroidManifest header to Android 6.0 and API Level 23 to allow it to use mitmproxy certificate")
				with codecs.open(manifest_file_path, 'r', 'utf-8') as f:
					manifest_file_contents = f.readlines()
				#globals.redirect_print_func(manifest_file_contents[0])
				...
				with codecs.open(manifest_file_path, 'w', 'utf-8') as f:
					manifest_file_contents = "".join(manifest_file_contents)
					f.write(manifest_file_contents)
					globals.redirect_print_func("\tSuccess: AndroidManifest header to Android 6.0 and API Level 23")
			...
			if self.inject_frida:
				globals.redirect_print_func("[I] Attempting to inject frida-gadget")
				try:
					#uncomment the follwing when installing a VR apk
					...
					globals.redirect_print_func("\tLaunchable Activity:" + launchable_activity)

				except IndexError:
					globals.redirect_print_func("No launchable activity found")
					sys.exit(1)
				...
				if not os.path.isfile(launchable_activity_path):
					globals.redirect_print_func("\tCould not find .smali file at %s" % launchable_activity_path)
					smali_file_name = launchable_activity_path.split(os.sep)[-1]
					tmp_apk_path = os.path.join(WORK_DIR, package_name)
					globals.redirect_print_func("\tSearching in apk directory %s for file %s: " % (tmp_apk_path, smali_file_name))
					for root, dirs, files in os.walk(tmp_apk_path):
						for tmp_file in files:
							if tmp_file == smali_file_name:
								...
								globals.redirect_print_func("\tFound .smali file at %s" % launchable_activity_path)

				globals.redirect_print_func("\t[I] Searching .smali")
				...
				globals.redirect_print_func("\t[I] Patching .smali")
